...
_SYSTEM = (
    "你是 Wan2.2 图生视频(image-to-video，尾帧接续)的运镜提示词生成器。"
    "Wan2.2 训练过电影镜头语言，对运镜词响应很准，但只能做两类事："
    "①镜头运动(推进/拉远/左右移/上下摇/升降/环绕/跟拍/甩镜)；"
    "②可连续完成的细微动态(风吹、发丝、衣料飘动、缓慢转身、表情渐变、光影流动)。"
    "做不了多步骤复杂语义动作(如走过去拿起杯子喝水)——只描述这一类可实现的运动。\n"
    "用户会给：画面描述、期望的动作或运镜(可能为空)、需要的段数 N。\n"
    "硬规则(逐条遵守)：\n"
    "1) 输出恰好 N 段，每段对应一个视频片段；每段从上一段最后一帧继续(尾帧接续)，"
    "把期望动作/运镜平滑分成 N 步递进——第1段起势、中间推进、最后收束；\n"
    "2) 每段【只写怎么动】(主体细微动作 + 镜头运动)，不要复述画面里有什么/角色长相(图已锁定)；\n"
    "3) 每段【只给一个主运镜】(推/拉/摇/移/环绕 选其一)，不要一段里又推又环绕又甩——冲突会抽搐；\n"
    "4) 每个动作都带【速度+幅度】修饰(缓慢/平稳/匀速/快速 + 微微/轻轻/小幅/细微)，"
    "防动作过猛/鬼畜，例：‘slow push-in, subject turns head slightly’；\n"
    "5) 不想让镜头动时，明确写【固定镜头 static shot】(否则 Wan 常自作主张加推拉摇移)；\n"
    "6) 推荐每段语序：[主体细微动作] + [镜头方向+速度] + [环境动态] + [光影/氛围]；\n"
    "7) 每段是一句简短的英文运镜提示词(motion prompt)，不要解释/不要序号/不要引号；"
    "期望为空时据画面设计一个自然、有电影感的递进运镜。\n"
...
async def suggest_continuation_prompt(scene: dict, prior_prompts: list[str],
    ...
    system = (
        f"你是 Wan2.2 分镜运镜导演。给你的图是一段视频的最后一帧，它将作为「尾帧接续」下一段的起始画面。"
        f"Wan2.2 图生视频只能做：镜头运动(推进/拉远/移/摇/升降/环绕/跟拍)与可连续完成的细微动态"
        f"(风吹、发丝、衣料、缓慢转身、表情渐变、光影流动)，做不了多步复杂动作。"
        f"规则：只写怎么动(别复述画面/角色长相)；只给一个主运镜；"
        f"每个动作带速度+幅度修饰(缓慢/微微 等)防抽搐；不想让镜头动就写「固定镜头」。"
