...
from comfy_core.providers.base import (
    VideoProvider,
    video_provider_registry,
)
from comfy_core.providers.wan22 import Wan22Provider
from comfy_core.providers.ltx import LtxProvider
...
if settings.COMFYUI_BASE_URL and settings.COMFYUI_VIDEO_AS:
    ...
    if _as:
        ...
        from comfy_core.providers.comfyui import ComfyUIProvider
...
if settings.LTX2_ENABLED and (settings.COMFYUI_LTX_BASE_URL or settings.COMFYUI_BASE_URL):
    from comfy_core.providers.comfyui_ltx import ComfyUILtxProvider
...
if settings.SULPHUR2_ENABLED and (settings.SULPHUR2_BASE_URL or settings.COMFYUI_BASE_URL):
    from comfy_core.providers.sulphur2 import Sulphur2Provider
...
if settings.COMFYUI_BASE_URL:
    from comfy_core.providers.comfyui_s2v import ComfyUIS2VProvider
    video_provider_registry.register(ComfyUIS2VProvider())
# Wan2.2-T2V 文生视频：隐藏 Provider，不进用户下拉，由「出片模式=t2v」路由。配了端点才注册。
if settings.COMFYUI_BASE_URL:
    from comfy_core.providers.comfyui_t2v import ComfyUIT2VProvider
...
if settings.STANDIN_ENABLED and settings.STANDIN_BASE_URL:
    from comfy_core.providers.standin import StandInT2VProvider
