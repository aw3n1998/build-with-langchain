...
>
> 技术栈：Python · FastAPI · LangGraph · React + Vite · lightx2v · Wan2.2-T2V · Pydantic V2 · SQLite

> **当前路线 = 纯文生视频 (t2v) + lightx2v（不走 ComfyUI）。** 分镜文本直接生成视频（不出图、不选图），人物一致靠自训的 **Wan-T2V 角色 LoRA**。
> 图生视频(i2v) / FLUX 出图 / 选图 / PuLID 锁脸 / S2V 对口型 / ReActor 换脸那套代码**保留为休眠退路**（端点门控、默认不暴露），本文下半部分的 ComfyUI 章节即属此类。
...
分镜表（角色圣经：每个角色固定外貌 + 固定音色 + 角色 LoRA，出片/配音自动注入）
  │  ② 每镜 Wan2.2-T2V-A14B 文生视频（lightx2v 后端；文本直接出片，无需出图/选图）
  ▼
分镜成片
...
GPU 重活（文生视频出片）走 **lightx2v 的 HTTP API**（Wan2.2-T2V-A14B 双专家），后端只做编排，端点未配置时自动休眠——本机零 GPU 也能跑完编排、改提示词、排分镜。
人物一致性来自**自训 Wan-T2V 角色 LoRA**（t2v 没有首帧，身份全靠 LoRA 锁定）。
...
- **文生视频出片**：Wan2.2-T2V-A14B 双专家，走 **lightx2v** 推理引擎（不依赖 ComfyUI）。分镜文本直接出竖屏短片，**4 步蒸馏 LoRA 极速档**先打样、满档精修两步走。
...
- **（休眠退路）i2v / 出图选图 / PuLID / 对口型 / 换脸**：FLUX 出图 + Wan2.2-I2V + S2V + ReActor 那套走 ComfyUI HTTP，代码保留、默认关闭，配 `COMFYUI_BASE_URL` 才启用。
...
### B. Colab 一条龙（推荐，自带 GPU，纯 t2v + lightx2v）

打开 **`colab_lightx2v_t2v.ipynb`**（纯文生视频、不装 ComfyUI）→ 运行时选 GPU（Blackwell / Hopper / Ada / A100）→ 菜单 **代码执行程序 → 全部运行 (Run all)**：

1. **§1** 自动探测显卡设 `cpu_offload`（>70G 双专家全 GPU；≤70G 卸载，兼容 5090 32G）。
2. **§3** 装 lightx2v（已固化 torch / 配置 / 注意力算子等实战坑）；**§4** 把 Wan2.2-T2V-A14B 权重 + 4 步蒸馏 LoRA 下到本地。
3. **§5** 起 lightx2v server（已自动给 server 插桩、可挖出片真 traceback）；**§6→§8→§9** 写 `.env` → 起后端 → cloudflared 公网地址。
4. 训好角色 LoRA 后填进 **§5d** 跑一次（把 `lora_configs` 写进 server config 并重起）。
5. 部署进度 / 待办 / 排错见仓库根的 **`lightx2v_t2v_部署状态.md`** 交接文档。

> **i2v / ComfyUI 一条龙（休眠退路）**：另有 `colab_deploy.ipynb`（FLUX 出图 + Wan2.2-I2V + S2V + 换脸，走自托管 ComfyUI）。当前产品默认纯 t2v，不需要它；下面「自托管 ComfyUI」章节即针对这条退路。
